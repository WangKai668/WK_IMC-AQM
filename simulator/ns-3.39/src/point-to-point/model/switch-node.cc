...
bool IsTcpAck(Ptr<Packet> p, const CustomHeader &ch, bool &isPureAck)
{
    isPureAck = false;
    // 快速判断：如果 CustomHeader 已经标记为 ACK/control
    if (ch.l3Prot == 0xFC || ch.l3Prot == 0xFD) {
        isPureAck = true;
        return true;
    }
    if (ch.l3Prot != 0x06) // 不是 TCP
        return false;
    ...
    uint8_t flags = th.GetFlags();
    bool isAck = (flags & TcpHeader::ACK) != 0; // or (flags & 0x10)

    uint32_t tcpHeaderSize = th.GetSerializedSize();
    uint32_t payloadLen = 0;
    if (cp->GetSize() > tcpHeaderSize)
        payloadLen = cp->GetSize() - tcpHeaderSize;

    isPureAck = isAck && (payloadLen == 0);
    return isAck;
}
...
void SwitchNode::SwitchNotifyEnqueue(uint32_t ifIndex, uint32_t qIndex, Ptr<Packet> p,CustomHeader &ch) {
	bool isPureAck;
	IsTcpAck(p, ch, isPureAck);

	if(m_mmu->aqmMode == PRED&&!isPureAck){
		if(m_mmu->isQlaStarted[ifIndex][qIndex] == false){
			m_mmu->StartQlaTimer(ifIndex, qIndex);
			m_mmu->isQlaStarted[ifIndex][qIndex] = true;
		}
	}
	
	// uint32_t cur_pkts = m_mmu->egress_pkts[ifIndex][qIndex];

    // // 记录统计量（仅累加）
    // m_mmu->RecordQlaPacketStats(ifIndex, qIndex, p->GetSize(), cur_pkts);
	...
	if(m_mmu->aqmMode == PRED){
		...
		SwitchMmu::FcsState &fcs = m_mmu->GetFcsState(ifIndex, qIndex);

		uint32_t sid = fingerPrint & ((1 << fcs.INDEX_SIZE) - 1);//对2^INDEX_SIZE取余数，得到位图id

		// 不在此处直接写入位图，改由 UpdateFcsStats 负责原子性读/写与计数
		if(!isPureAck){
			m_mmu->UpdateFcsStats(ifIndex, qIndex, fingerPrint, Simulator::Now().GetNanoSeconds());
		}
