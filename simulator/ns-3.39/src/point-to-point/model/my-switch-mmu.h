#ifndef MY_SWITCH_MMU_H
#define MY_SWITCH_MMU_H
...
class MySwitchMmu: public Object {
public:
	...
	void SetThresholdScheme(uint32_t port, TypeId tcpTypeId, uint64_t C, Time RTT, Time propagationDelay, Time transmissionDelay);
	void SetThresholdCEDM(uint32_t port, TypeId tcpTypeId, uint64_t C, Time RTT, Time propagationDelay, Time transmissionDelay);
	void SetThresholdMBECN(uint32_t port, TypeId tcpTypeId, uint64_t C, Time RTT, Time propagationDelay, Time transmissionDelay);
	void SetThresholdSTD(uint32_t port, TypeId tcpTypeId, uint64_t C, Time RTT, Time propagationDelay, Time transmissionDelay);
	void ConfigEcn(uint32_t port, uint32_t _kmin, uint32_t _kmax, double _pmax);
	void SetThresholdMyAlg(uint32_t port, TypeId tcpTypeId, uint64_t C, Time RTT, Time propagationDelay, Time transmissionDelay);
	void ConfigECNThreshold(uint32_t port);
	...
	void SetECNMarkingAlg(uint32_t alg);

	void SetRTT(Time rtt);

	bool EnqueueScheme(uint32_t ifIndex, uint32_t qIndex, Ptr<Packet> p, double avg_s, double s);

	bool EnqueueCEDM(uint32_t ifIndex, uint32_t qIndex, Ptr<Packet> p, double avg_s);

	bool EnqueueMBECN(uint32_t ifIndex, uint32_t qIndex, Ptr<Packet> p, double avg_s);

	bool EnqueueSTD(uint32_t ifIndex, uint32_t qIndex, Ptr<Packet> p, double avg_s);

	bool EnqueueBurst(uint32_t ifIndex, uint32_t qIndex, Ptr<Packet> p, double avg_s, double s);

	bool DequeueCEDM(uint32_t ifIndex, uint32_t qIndex, Ptr<Packet> p, double avg_s);

	bool DequeueMBECN(uint32_t ifIndex, uint32_t qIndex, Ptr<Packet> p, double avg_s);

	bool DequeueScheme(uint32_t ifindex, uint32_t qIndex, Ptr<Packet> p, double avg_s);

	bool DequeueSTD(uint32_t ifIndex, uint32_t qIndex, Ptr<Packet> p, double avg_s);

	bool DequeueBurst(uint32_t ifIndex, uint32_t qIndex, Ptr<Packet> p, double avg_s);

	double GetECNMarkingRaatio();
	...
	// MBECN 参数
	// 每个队列的阈值
	uint32_t m_kmax[pCnt][qCnt], m_kmin[pCnt][qCnt], m_kbase[pCnt][qCnt];
	// 每个端口的阈值
	uint32_t kport[pCnt];
	...
	// ECN Marking Algorithm
	uint32_t ecnMarkingAlg;
	 ...
	 */
	// 流状态
	enum FlowState { IDLE, IDENTIFY, BURST, CONGESTION };
	...
	struct QueueWindow {
		...
		// 窗口长度
		Time windowLength = Seconds(0);
		// 上次梯度
		double lastGradient = 0.0;
		// 当前梯度
		double currentGradient = 0.0;
		// 剩余突发数据量
		uint32_t remainingBurst = 0;
		// 初始梯度
		double initialGradient = 0.0;
		// 初始队列长度
		uint32_t initialQueueLength = 0;
		// 原阈值
		uint32_t originalThreshold = 0;
    };

    QueueWindow queueWindow[pCnt][qCnt];
...
private:
	std::ofstream threshold_file;  // 用于记录MBECN阈值变化的文件流
};
