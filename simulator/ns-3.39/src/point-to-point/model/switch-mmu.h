...
public:
	static double TFCS_FACTOR;//论文原文采用TFCS=1.25*RTT
	static double TQLA_FACTOR;//论文原文采用TQLA=5*RTT
	static double LAMBDA_MIN;//λ取值（0~正无穷），设置最小取值，原文采用0.05

	static bool EnableQLA;//是否启用QLA算法，默认启用
	static bool EnableFCS;//是否启用FCS算法，默认启用
	...
	//用于初始化PRED算法的静态参数【使用算法默认值】
	void initPredStatics(){
		TFCS_FACTOR = 1.25;
		TQLA_FACTOR = 5;
		LAMBDA_MIN = 0.05;
	}
	
	//用于初始化PRED算法的静态参数【使用自定义参数】
	void initPredStatics(double tfcs_factor, double tqla_factor, double lambda_min){
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	class FcsState {
	public:
		uint32_t INDEX_SIZE = INIT_SIZE;// 位图初始化大小
		uint64_t last_reset_ns;      // 上次窗口重置时间
		uint32_t n_current;          // 当前窗口新流计数，论文中的n
		uint32_t n_last;             // 上一个窗口新流计数，论文中的nlast
		uint32_t interval_seq;       // 窗口序号
		std::vector<uint64_t> bitmap; // 指纹位图（大小 2^IndexSize）

		uint32_t f_n;				 // 每个周期更新一次，上周期余下的f(N)数值，默认为1
		uint32_t f_n_min;			 // f(N)最小值，不能比它小，论文没说，默认为1
		uint32_t n_big;				 // 论文中的“N”，N=max(n,nlast);

		FcsState() : last_reset_ns(0), n_current(0), n_last(0), interval_seq(0),f_n(1),f_n_min(1),INDEX_SIZE(INIT_SIZE) {
			initiate();
		}

		//传入位图大小的构造函数，包含INDEX_SIZE的赋值
		FcsState(int length) : last_reset_ns(0), n_current(0), n_last(0), interval_seq(0),f_n(1),f_n_min(1),INDEX_SIZE(length) {
			initiate(length);
		}
	...
	public:
		...
		public:
			...
			void DoMDFromFCS(FcsState& fcs){
				lambda_base *= fcs.getFn();
				if(lambda_base>=1)
					std::cout<<"[ERROR]DoMDFromFCS==> lambda_base "<<lambda_base
					<<" F(N) "<<fcs.getFn()<<std::endl;
			}

			//执行相位
			void doPhaseWork(FcsState& fcs){
				IncreasePhase();//相位自增，从上一个切换到当前

				if(Is4PhaseEnd()){//如果是0相位，那么要么是第一次开始，要么是上个周期结束
					DecisionMaker();
					// DoMDFromFCS(fcs);
				}
			...
			//获取平均队列长度
			double getAvgQ(){
				return (double)phase_qlen_sum / phase_qlen_cnt;
			}

			//获取平均吞吐量
			double getAvgThroughputByTqlaNs(double Tqla_ns){
				return 8 * (double)phase_tx_bytes / (Tqla_ns / 1e9);//Bps转换为bps
			}

			//请在dophasework之前调用
			void setUtility(double utility){
				phase_utils[test_phase] = utility;
			}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//RED算法相关函数声明
	double Point_Slope_Lambda = 2;//点斜式的lambda
	bool Use_Point_Slope = false;//是否使用点斜式，默认不使用
	//是否启用点斜式
	void UsePointSlope(bool use){
		Use_Point_Slope = use;
	}
	//设置点斜式lambda
	void SetPointSlopeLambda(double lambda){
		Point_Slope_Lambda = lambda;
	}
	...
	bool isQlaStarted[pCnt][qCnt] = {false}; //QLA是否已启动

	//单位是纳秒（ns）
	double portRTTs[pCnt]={0};
	...
	double GetPortRTT_ns(uint32_t port){
		// std::cout<<"获取端口"<<port<<"的RTT，值为："<<portRTTs[port]<<"ns"<<"\n";
	...
	// QLA出队字节数统计
	void RecordQlaDequeueBytes(uint32_t port, uint32_t queue, uint32_t pktSize);

	//更新端口-队列的QLA状态
	void UpdateQlaStatsDrivenByTime(uint32_t port, uint32_t queue, /*uint32_t pktSize, uint32_t curQlenPkts, */ uint64_t now_ns/*, uint64_t rtt_ns*/);

	//记录QLA数据
	void RecordQlaPacketStats(uint32_t port, uint32_t queue, /*uint32_t pktSize,*/ uint32_t curQlenPkts);

	//启动所有QLA定时器
	void StartAllQlaTimers();

	//启动某端口QLA定时器
	void StartQlaTimerOfPort(uint32_t port);

	//启动某端口某队列的QLA定时器
	void StartQlaTimer(uint32_t port, uint32_t queue);

	//设置自定义带宽
	void SetMyBandWidth_GBPS(uint64_t bw){
		MyBandWidth_GBPS = bw;
		for (uint32_t portId = 0; portId < pCnt; portId++) {
			bandwidth[portId] = MyBandWidth_GBPS * 1e9;
		}
	}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////PUBLIC添加结束//////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	// YRNK
	// 新增包数量数组
	uint64_t egress_pkts[pCnt][qCnt];

	// YRNK
	// 新增自定义带宽
	uint64_t MyBandWidth_GBPS = 25;
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
