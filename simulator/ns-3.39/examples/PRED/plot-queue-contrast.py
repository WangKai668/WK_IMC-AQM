...
def parse_args():
    ...
    p.add_argument("--alg1", default="PRED", help="Algorithm 1 name, e.g., RED/PIE/CoDel (default: RED)")
    p.add_argument("--alg2", default=None, help="Algorithm 2 name (optional), e.g., RED/PIE/CoDel (default: None)")
...
log_file1 = f"{args.dump_dir}/evaluation-{args.alg1}.out"
log_file2 = f"{args.dump_dir}/evaluation-{args.alg2}.out" if args.alg2 else None
# 初始化数据存储
time_ecn = []
...
# 实现间隔性采样
plt.plot(time_queue[::step], queue_lengths[::step], marker="o", linestyle="-", color="blue", label=f"Queue Length (KB)-{args.alg.rsplit('-', 1)[0]}")#按-分开，去除最后一项的时间戳
plt.plot(time_queue1[::step], queue_lengths1[::step], marker="o", linestyle="-", color="orange", label=f"Queue Length (KB)-{args.alg1.rsplit('-', 1)[0]}")
if log_file2:
    plt.plot(time_queue2[::step], queue_lengths2[::step], marker="o", linestyle="-", color="green", label=f"Queue Length (KB)-{args.alg2.rsplit('-', 1)[0]}")
# plt.plot(time_queue, queue_lengths, marker="o", linestyle="-", color="orange", label="Queue Length (KB)")
plt.xlabel("Time (ms)")
plt.ylabel("Queue Length (KB)")
if log_file2:
    title = f"({args.alg.rsplit('-', 1)[0]}), ({args.alg1.rsplit('-', 1)[0]}) and ({args.alg2.rsplit('-', 1)[0]}) Switch Port Queue Length Over Time"
else:
    title = f"({args.alg.rsplit('-', 1)[0]}) and ({args.alg1.rsplit('-', 1)[0]}) Switch Port Queue Length Over Time"
plt.title(title)
plt.legend()

output_file_2 = f"{args.dump_dir}/output_queue_length-{args.alg}-contrast-{args.alg1}"
if args.alg2:
    output_file_2 += f"-contrast-{args.alg2}"
output_file_2 += ".png"
...
with open(f"{output_prefix}-alg-time_queue.txt", "w") as f:
    for t, length in zip(time_queue, queue_lengths):
        f.write(f"{t},{length}\n")

with open(f"{output_prefix}-alg1-time_queue.txt", "w") as f:
    for t, length in zip(time_queue1, queue_lengths1):
        f.write(f"{t},{length}\n")

if log_file2:
    with open(f"{output_prefix}-alg2-time_queue.txt", "w") as f:
        for t, length in zip(time_queue2, queue_lengths2):
            f.write(f"{t},{length}\n")
