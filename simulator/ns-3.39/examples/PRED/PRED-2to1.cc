...
double pause_time = 5, simulator_stop_time = 0.001;//3.01;
...
int main(int argc, char *argv[])
{
	...
	begint = clock();
		...
		std::string confFile = "/home/wk/Reverie-Platform/ns3-datacenter/simulator/ns-3.39/examples/PRED/config-PRED-2to1.txt";
	...
	for (uint32_t i = 0; i < link_num; i++)
	{
		...
		// sw->m_mmu->SetPortRTT(src,Time(link_delay).GetNanoSeconds);
		...
		if (n.Get(i)->GetNodeType()) { // is switch
			...
			for (uint32_t j = 1; j < sw->GetNDevices(); j++) {
				...
				for (uint32_t qu = 0; qu < 8; qu++){
					...
					//拥塞控制算法选择！
					//////////////////////////////////////////////////////////////////////////////////////////////////////////
					std::cout<<"YRNK-->选择拥塞控制算法"<<"\n";
					switch(cc_mode){
						case RED_CC_MOD:				//如果拥塞控制模式是RED模式！！！
							std::cout<<"采用RED算法"<<"\n";
							//设置拥塞模式
							sw->m_mmu->SetEcnMode(RED_CC_MOD);
							//启用RED相关操作
							// 使用命令行参数或默认值
							...
							break;
						case PRED_CC_MOD:				//如果拥塞控制模式是PRED模式！！！
							std::cout<<"采用PRED算法"<<"\n";
							...
							//设置拥塞模式
							sw->m_mmu->SetEcnMode(PRED_CC_MOD);
							//启用PRED相关操作
