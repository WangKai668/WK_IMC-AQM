# -*- coding: utf-8 -*-
# 生成微观性能分析（Microscopic View）场景的拓扑文件和流量文件
# 配置：16 发送端 -> 1 交换机 -> 1 接收端，10 Gbps 链路，10 µs 延迟
# 流量时间窗口：
#   - 4 条流：0 ms → 80 ms（持续长流，包数极大）
#   - 12 条流：20 ms → 40 ms（窗口流，包数适中，20 ms 内可发完）
#   - 4 条流：60 ms → 80 ms（窗口流，包数适中）
...
# 长流包数：极大值（保证仿真期间不会发完）
LONG_FLOW_PACKETS = 100_000_000_000   # 10^11
...
def generate_topology():
    ...
    with open("topology-MV.txt", 'w') as f:
    ...
    print(f"✅ 拓扑文件已生成: topology-MV.txt")
...
def generate_flow():
    ...
    for start, end, count, ftype in WINDOWS:
        for i in range(count):
            ...
            flow_entries.append((src, RECEIVER_ID, dport, packets, start))

    # 写入流量文件
    with open("flow-MV-16to1.txt", 'w') as f:
        f.write(f"{len(flow_entries)}\n")
        for src, dst, dport, packets, start in flow_entries:
            f.write(f"{src} {dst} {PRIORITY} {dport} {packets} {start}\n")

    print(f"✅ 流量文件已生成: flow-MV-16to1.txt (共 {len(flow_entries)} 条流)")
